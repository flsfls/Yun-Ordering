...
import AddPuchaseLibrary from '@libraryView/AddPuchaseLibrary'; // eslint-disable-line
...
class PruchaseOrder extends React.Component {
  ...
  componentDidMount() {
    ...
    post('wap/department/selectList', {}, {}).then(({ data }) => {
      const newData = data.map((item) => {
        item.value = item.fsDepartmentId;
        item.label = item.fsDepartmentName;
        return item;
      })
      this.props.puchaseStore.addFsSupplierList(newData, 'department');
    });
  ...
  getOrderList = (flag, data) => {
    ...
    // 向后台发送请求
    post(`wap/picking/list`, {}, { ...data }).then(({ data }) => {
  ...
  showActionSheet = () => {
    ...
    ActionSheet.showActionSheetWithOptions(
      ...
      (buttonIndex) => {
        ...
        if (buttonIndex === 0) {
          // 把入品标识设置为3
          sessionStorage.enterFlag = '6';
          this.addBefore({ fiBillKind: 1 }, 1);
          // 如果是采购退货单的话
        } else if (buttonIndex === 1) {
          // 把入品标识设置为4
          sessionStorage.enterFlag = '7';
  ...
  addBefore = (data, enterFlag) => {
    return post('wap/picking/beforeAdd', data, {}).then(({ data }) => {
      delete data.purchasedtlList;
      delete data.stockdtlList;
      const { getOrder } = sessionStorage;
      if (enterFlag === 1 && getOrder) {
        data = Object.assign({}, data, JSON.parse(getOrder));
      }
      sessionStorage.chooseOrder = JSON.stringify(data);
      this.props.history.push({ pathname: '/home/libraryPost/addPuchaseLibrary' });
    })
  }
  ...
  listItemHandler = (fiId, orderItem, index) => {
    ...
    post(`wap/picking/beforeAdd`, { fiId, fiBillKind }).then(({ data }) => {
      // 如果采购订单列表进入的话,标识为2

      sessionStorage.enterFlag = '8';
      sessionStorage.enterMaterial = JSON.stringify(data.pickingdtlList.colContent);
      if (fiBillStatus === 0) {
        sessionStorage.status = '1'
      } else if (fiBillStatus === 1) {
        sessionStorage.status = '2'
      }
      const chooseMaterial = data.pickingdtlList.colContent.map((item) => {
        const {
            fsUnitIdSelected,
          fsUnitId,
          fsUnitName,
  ...
  render() {
    ...
    return (
      <div className="purchaseOrder inner_body">
        ...
        />
        <Route path="/home/libraryPost/addPuchaseLibrary" component={AddPuchaseLibrary} />
      </div>
    );
  }
}
